...
object MyModule {
  ...
  def fib(n: Int): Int = {
    ...
    @tailrec
    def fib(n: Int, fib0: Int, fib1: Int): Int = n match {
      case 0 => fib0
      case 1 => fib1
      case _ => fib(n - 1, fib1, fib0 + fib1)
    }
...
object PolymorphicFunctions {
  ...
  def isSorted[A](as: Array[A], gt: (A,A) => Boolean): Boolean = {
    def lastIndex(i: Int) = i >= as.length - 1
    
    @tailrec
    def recSorted(i: Int, result: Boolean): Boolean = {
      if (lastIndex(i)) result
      else recSorted(i + 1, gt(as(i), as(i + 1)))
    }
    recSorted(0, true)
  }
